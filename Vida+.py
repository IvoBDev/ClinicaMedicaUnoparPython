...
def estatisticas():
    ...
    print(f"Paciente mais novo: {min(idades)} anos")
    print(f"Paciente mais velho: {max(idades)} anos")
    sleep(2)


# Opção 3 - Função Buscar Paciente (Busca Parcial e Insensível a Maiúsculas)
def buscar_paciente():
    if not pacientes:
    ...
    termo_busca = input("\nDigite o nome (ou parte do nome) do paciente: ").strip().lower()
